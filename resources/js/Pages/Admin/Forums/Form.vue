...
const props = defineProps({ forum: Object, categories: Array });
...
const submit = () => {
    if (isEdit) form.put(route('admin.forums.update', props.forum.id));
    else form.post(route('admin.forums.store'));
};
</script>

<template>
    <Head :title="isEdit ? 'Admin · Edit Forum' : 'Admin · New Forum'" />
    <AdminLayout>
        <header class="mb-8">
            <p class="vx-meta mb-2">{{ isEdit ? 'Editing forum' : 'New forum' }}</p>
            <h1 class="font-serif text-4xl font-semibold tracking-tight" style="font-family:'Fraunces',serif;color:var(--text)">
                {{ isEdit ? forum.name : 'Untitled' }}
            </h1>
        </header>

        <form @submit.prevent="submit" class="space-y-6 max-w-2xl">
            <div>
                <label class="vx-meta mb-2 block">Category</label>
                <select v-model="form.category_id" class="vx-input" required>
            ...
            <div class="flex justify-end gap-2 pt-4 border-t" style="border-color:var(--border)">
                <Link :href="route('admin.forums.index')" class="vx-btn-secondary">Cancel</Link>
